...
int main(int argc, char **argv)
{
    ...
    gettimeofday(&end_time, NULL); //레코드 읽고 난 후의 시간 저장
    
    timecost = ((end_time.tv_sec - start_time.tv_sec)*1000000) + ((end_time.tv_usec - start_time.tv_usec)); //timecost 계산(us단위)

    
    printf("#records: %d timecost: %d us\n",records, timecost); //레코드 개수와 timecost출력 후 종료

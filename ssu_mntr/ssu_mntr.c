...
void ssu_mntr() {
    ...
    //log.txt파일생성
    memset(idDir, 0, BUFLEN);
    strcpy(idDir, saved_path);
    strcat(idDir, "/20180753");
    if (!access(checkDir, F_OK)) { //내 학번 디렉토리 없는경우 새로생성
        mkdir(idDir, 0644);
    }
    memset(logFile, 0, BUFLEN);
    strcpy(logFile, saved_path);
